...
/// List all seats (in-memory demo)
#[handler]
pub async fn list_seats() -> Json<Vec<seat::Model>> { let seats = SEATS.read().await; Json(seats.clone()) }
...
/// Fetch seats from external service (demo using reqwest)
#[handler]
...
/// Sync all seats to external service (demo)
#[handler]
pub async fn sync_all_seats() -> Json<serde_json::Value> { let seats = SEATS.read().await; let mut success_count = 0; let mut error_count = 0; for seat in seats.iter() { if sync_seat_to_external(seat).await.is_ok() { success_count += 1 } else { error_count += 1; tracing::warn!(seat_id=seat.id, "Failed to sync seat"); } } Json(json!({"synced": success_count, "errors": error_count, "total": seats.len()})) }
