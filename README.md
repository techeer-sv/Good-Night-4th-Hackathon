...
- [x] 코드 품질 보장
    - backend
        - 단위 테스트: 레이어를 고립해서 빠르게 검증
            - SeatRepository 테스트: H2 메모리 DB로 JPA CRUD/정렬 검증
        - 통합 테스트: 컨테이너 전체 구동
            - SpringBootTest + 랜덤 포트 + TestRestTemplate로 실제 HTTP 호출
            - H2로 애플리케이션 구동, SeatInitializer 시드 포함한 end-to-end 확인
    - frontend
        - 타입 체크: Typescript
        - 린팅: Eslint
...
- [x] 사용자 경험 개선
    - 직관적인 UI
        - 빨초파로 예약여부 및 선택 여부 표현
    - 네트워크 지연이 발생했을 때 편의성
        - 로딩 인디케이터(스피너, 프로그레스 바) 표시
    - 예약이 실패했을 때 편의성
        - 예약된 좌석 선택 시 메세지 제공
    - 모바일에서 접속했을 때 편의성
        - 반응형 적용
- [x] 안정적인 서비스 운영
    - 잘못된 요청이 들어왔을 때
        - 서버단에서 Request Validation/잘못된 요청 시 HTTP 400
    - 존재하지 않는 좌석을 예약하려 할 때
        - 프론트에서 UI로 존재하지 않는 좌석 선택 불가하게 설정
    - 데이터 정합성 보장
        - DB 제약조건 설정
