...
-리액트
    가상DOM을 사용한 빠른 렌더링
-Node.js + express
    빠른 서버 사이드 개발과 REST API 구현이 쉬움
-SQLite
    서버 설치나 별도의 설정이 필요 없는 가벼운 파일 기반 데이터베이스로, 소규모 프로젝트에서 빠르게 데이터 관리를 할 수 있어 선택
###구현한 요구사항 체크리스트
1. **좌석 현황 표시**
...
2. **좌석 예약 기능능**
-예매된 자리에 마우스를 올릴 경우 마우스 포인터의 모양이 금지 표시로 바뀜.
[]예약 성공/실패에 대한 명확한 피드백 제공
-예약 성공시 팝업이 뜨면서 예약자 이름과 좌석 번호를 알려줌.
-예약 실패시 
