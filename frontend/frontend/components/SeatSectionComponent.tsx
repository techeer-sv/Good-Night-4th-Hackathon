// Deprecated duplicate (moved to src/components/seats/SeatSectionComponent.tsx)
"use client";
import React from 'react';
import { SeatComponent } from '../hooks/SeatComponent';
import type { SeatSection, Seat } from '../src/types/seat';
        ...
        {sortedRows.map(rowLetter => {
          ...
          return (
            <div key={`${section.id}-${rowLetter}`} className="flex items-center justify-center gap-1">
              ...
              {/* Row label (mirrored on right for longer rows) */}
              {rowSeats.length > 20 && (
