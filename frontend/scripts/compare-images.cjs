const path = require('path');
const looksSame = require('looks-same');

const appImg = path.resolve(process.cwd(), 'test-results', 'visual', 'app-select.png');
const exampleImg = path.resolve(process.cwd(), 'test-results', 'visual', 'example-stitch.png');
const diffOut = path.resolve(process.cwd(), 'test-results', 'visual', 'diff.png');

looksSame(appImg, exampleImg, { tolerance: 5 }, (err, { equal }) => {
  if (err) {
    console.error('Comparison error', err);
    process.exit(1);
  }
  if (equal) {
    console.log('Images are the same.');
    process.exit(0);
  } else {
    console.log('Images are different.');
    looksSame.createDiff({
      reference: appImg,
      current: exampleImg,
      diff: diffOut,
      highlightColor: '#ff00ff',
      tolerance: 5,
    }, function(err) {
      if (err) {
        console.error('Diff creation error', err);
        process.exit(1);
      }
      console.log('Wrote diff to', diffOut);
      process.exit(1); // Exit with a non-zero code to indicate a difference
    });
  }
});
